...
public class UtilityCommands extends PermissionsCommand {
	...
	public void reload(PermissionsEx plugin, CommandSender sender, Map<String, String> args) {
		...
		PexDashboard dashboard = plugin.getDashboard();
		if (dashboard != null && dashboard.isEnabled()) {
			try {
				int loaded = dashboard.getPasswordStorage().loadAndHash();
				sender.sendMessage(ChatColor.WHITE + "Loaded " + loaded + " Dashboard users.");
			} catch (IOException | InvalidConfigurationException | IllegalStateException e) {
				sender.sendMessage(ChatColor.RED + "Failed to reload dashboard passwords! Check configuration!\n" + ChatColor.RED + "Error (see console for full): " + e.getMessage());
				plugin.getLogger().log(Level.WARNING, "Failed to reload dashboard passwords when " + sender.getName() + " ran `pex reload`", e);
			}
		}
	}
	...
	public void convertUUID(final PermissionsEx plugin, final CommandSender sender, Map<String, String> args) {
		...
		final Iterator<List<String>> splitIdentifiers = Iterables.partition(userIdentifiers, 50 * 1000).iterator(); // 50k
		// users
		// per
		// 10
		// minutes
